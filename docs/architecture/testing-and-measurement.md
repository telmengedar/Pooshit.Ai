...
| `SequenceRng` | An ordered script per drawing method. | An ordered log of `(method, requestedBound)` for every call. | Throws on an unscripted method, on script exhaustion, and on a scripted value outside the requested bound. |
...
| Negative paths | Invalid population size; empty training sample set; a sample key naming a neuron that does not exist (today: index `-1`, surfacing as an out-of-range access deep inside evaluation); a malformed serialised stream. | — |
...
| Fresh-blood band (`Mutate` strategy) | With a `Generator` returning a marked chromosome, the number of slots carrying the marker equals `Elitism`. **This currently fails** — the band is `i > trainingBuffer.Length - setup.Elitism`, spanning `Elitism − 1` slots (#9054). Per R5 the assertion is written to the intended contract and `[Ignore("DiVoid #9054")]`d. |
| Rivalism | With `Rivalism = r`, exactly `r` candidates are evaluated per slot (assert via the stub evaluator's call log) and the best is kept. |
